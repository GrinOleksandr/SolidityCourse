...
abstract contract VRFConsumerBase is VRFRequestIDBase {
  ...
  function initializeVRFConsumerBase(address _vrfCoordinator, address _link) internal {
    vrfCoordinator = _vrfCoordinator;
    LINK = LinkTokenInterface(_link);
  }
  ...
  BaseRelayRecipient
{
  ...
  function addToQueue(
    uint8 _operationTypeId,
    address sender,
    uint256 _amount,
    bytes32 _randomnessRequestId,
  ...
  function request_ETH_USD_price() public returns (bytes32 requestId) {
    ...
    request.add(
      'get',
      'https://min-api.cryptocompare.com/data/pricemultifull?fsyms=ETH&tsyms=USD&api_key=8ba055208fcf46cf3a795f380fa1b48083944638d81276bcef280b6a00328063'
    );
  ...
  function _buyTokensForDAI(
    ...
    if (
      DAITokenContract.balanceOf(sender) <= amount ||
      myTokenContract.balanceOf(address(this)) <= amountOfTokensToBuy ||
      allowance <= amount
    ) {
      return;
    }
  ...
  function _msgSender() internal view override(ContextUpgradeable, BaseRelayRecipient) returns (address ret) {
  ...
  function _msgData() internal view override(ContextUpgradeable, BaseRelayRecipient) returns (bytes calldata ret) {
    if (msg.data.length >= 20) {
